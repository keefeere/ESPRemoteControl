...
NAME = "90-inpudeck-le.conf"
LEGACY_NAME = "90-inpudeck-le-test.conf"
OLD_NAME = "90-esp-remote-le.conf"
OLD_LEGACY_NAME = "90-esp-remote-le-test.conf"
DAEMONS = ("/usr/libexec/bluetooth/bluetoothd", "/usr/lib/bluetooth/bluetoothd")
MARKER = "# Managed by InpuDeck: inpudeck-le-setup.py\n"
OLD_MARKER = "# Managed by ESP Remote: linux-le-setup.py\n"
...
def read_owned(path, legacy=False, old_brand=False):
    if path.is_symlink():
    ...
    if old_brand:
        allowed = {value.replace(MARKER, OLD_MARKER, 1) for value in allowed}
    if legacy:
...
class Setup:
    def __init__(self, root=Path("/"), run=command):
        ...
        self.old_persistent = self.persistent.with_name(OLD_NAME)
        self.old_runtime = self.runtime.with_name(OLD_NAME)
        self.old_legacy = self.runtime.with_name(OLD_LEGACY_NAME)

    def status(self):
        for label, path in (("Persistent", self.persistent), ("Temporary", self.runtime),
                            ("Earlier temporary test", self.legacy),
                            ("ESP Remote persistent", self.old_persistent),
                            ("ESP Remote temporary", self.old_runtime),
                            ("ESP Remote earlier test", self.old_legacy)):
        ...
        print("Use inpudeck-hid.sh --why to check the actual LE API on the paired device.")

    def plan(self, action, temporary):
        paths = (self.persistent, self.runtime, self.legacy,
                 self.old_persistent, self.old_runtime, self.old_legacy)
        old = {path: read_owned(
                    path,
                    path in (self.legacy, self.old_legacy),
                    path in (self.old_persistent, self.old_runtime, self.old_legacy),
                ) for path in paths}
        new = dict(old)
        new[self.old_persistent] = None
        new[self.old_runtime] = None
        new[self.old_legacy] = None
        if action == "enable":
