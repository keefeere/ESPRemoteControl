#!/usr/bin/env python3
# Managed by ESP Remote: Bluetooth audio receiver toggle
...
MARKER = "# Managed by ESP Remote: Bluetooth audio receiver toggle\n"
NAME = "90-esp-remote-audio-receiver.conf"
...
def install(prefix, data):
    script = prefix / "libexec/esp-remote-control/linux-audio-receiver.py"
...
def uninstall(prefix, data, receiver):
    paths = (prefix / "libexec/esp-remote-control/linux-audio-receiver.py",
...
def main():
    ...
    state = Path(os.environ.get("XDG_STATE_HOME") or Path.home() / ".local/state") / "esp-remote-control"
    ...
    try:
        ...
        if args.action == "install":
            ...
            uninstall(prefix, data, receiver)
